...
from PySide6.QtWidgets import (QWidget, QVBoxLayout, QScrollArea, QFrame,
                               QHBoxLayout, QLabel, QGridLayout, QApplication, QInputDialog, QMenu, QLayoutItem)
...
class ClickableFrame(QFrame):
    def __init__(self, copy_text, parent=None):
        super().__init__(parent)
        self.copy_content = copy_text
        self.setCursor(Qt.PointingHandCursor)

    def mouseReleaseEvent(self, event):
        if event.button() == Qt.LeftButton:
            QApplication.clipboard().setText(self.copy_content)
            InfoBar.success("已复制", self.copy_content, parent=self.window())
        super().mouseReleaseEvent(event)


class GradientPreview(ClickableFrame):
    def __init__(self, colors, angle, parent=None):
        css = f"background: linear-gradient({angle}deg, {', '.join(colors)});"
        super().__init__(css, parent)
        self.colors = colors;
        self.angle = angle
        self.setStyleSheet(
...
class PaletteCard(QFrame):
    def __init__(self, item, parent=None):
        ...
        self.setStyleSheet(
            "PaletteCard { background-color: white; border: 1px solid #e0e0e0; border-radius: 8px; } PaletteCard:hover { border: 2px solid #009faa; }")
    ...
    def show_color_menu(self, pos, hex_code):
        menu = QMenu(self)
        menu.setStyleSheet(
            "QMenu { background-color: white; border: 1px solid #e0e0e0; border-radius: 8px; padding: 5px; } QMenu::item { padding: 6px 20px; border-radius: 4px; color: #333; } QMenu::item:selected { background-color: #f0f0f0; color: black; }")
        menu.addAction(f"复制 {hex_code}").triggered.connect(lambda: self.copy_text(hex_code))
    ...
    def copy_all(self):
        self.copy_text(", ".join(self.colors))

    def copy_text(self, text):
        QApplication.clipboard().setText(text); InfoBar.success("已复制", text, parent=self.window())

    def delete_me(self):
        CollectionService.remove_item(self.item['id']); self.deleteLater(); InfoBar.success("已删除", "色板已移除",
                                                                                            parent=self.window())
...
class FavPage(QScrollArea):
    def __init__(self):
        ...
        header.addStretch(1)
        self.btn_add = ToolButton(FluentIcon.ADD, self);
        self.btn_add.setToolTip("新建空色板");
        self.btn_add.clicked.connect(self.create_new_palette);
        header.addWidget(self.btn_add)
        icon_sync = getattr(FluentIcon, 'SYNC', FluentIcon.UPDATE)
        self.btn_refresh = TransparentToolButton(icon_sync, self);
    ...
    def create_new_palette(self):
        ...
        if ok and name: CollectionService.create_custom_palette(name); self.load_data()
    ...
    def load_data(self):
        ...
        for i in reversed(range(self.main_layout.count())):
            ...
            w = item.widget()
            if w and isinstance(w, BodyLabel): w.deleteLater()
    ...
    def add_color_card(self, item):
        ...
        v.setSpacing(0)
        color_area = ClickableFrame(item['hex'])
        ...
        info = self.create_info_area(item['name'], item['hex'], card, item['id'], copy_content=item['hex'])
        v.addWidget(info)
        self.content_flow.addWidget(card)
    ...
    def add_gradient_card(self, item):
        ...
        css = f"background: linear-gradient({item['angle']}deg, {', '.join(item['colors'])})"
        info = self.create_info_area(item['name'], "渐变色", card, item['id'], copy_content=css)
        v.addWidget(info)
        self.content_flow.addWidget(card)
    ...
    def create_info_area(self, title, subtitle, parent_card, item_id, copy_content=None):
        widget = QWidget();
        ...
        btn_style = "background: transparent; border: none;"
        if copy_content:
            btn_copy = TransparentToolButton(FluentIcon.COPY, widget);
            btn_copy.setFixedSize(28, 28);
            btn_copy.setIconSize(QSize(14, 14))
            btn_copy.setToolTip("复制值");
            btn_copy.setStyleSheet(btn_style);
            btn_copy.clicked.connect(lambda: self.copy_text(copy_content))
